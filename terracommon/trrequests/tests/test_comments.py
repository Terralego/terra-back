...
class CommentsTestCase(TestCase, TestPermissionsMixin):
    ...
    def test_comment_retrieval_with_only_internal_permission(self):
        self._set_permissions([
            'can_internal_comment_requests',
        ])
        for _ in range(2):
    ...
    def test_comment_creation_with_attachment(self):
        ...
        self.assertEqual(mimetypes.guess_type(tmp_file.name),
                         mimetypes.guess_type(comment_updated.filename))
